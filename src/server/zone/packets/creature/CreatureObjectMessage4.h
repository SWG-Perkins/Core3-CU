...
: BaseLineMessage(creo->getObjectID(), 0x4352454F, 4, 0x10) {
		...
		int size = skillMods->size();

		insertInt(size);
		insertInt(skillMods->getUpdateCounter());

		for (int i = 0; i < size; ++i) {
			String key = skillMods->getKeyAt(i);
			int value = creo->getSkillMod(key);
		...
		// Abilities
		PlayerObject* play = creo->getPlayerObject();
