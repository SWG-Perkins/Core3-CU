...
void QueueCommand::onStateFail(CreatureObject* creature, uint32 actioncntr, uint32 actionCRC) {
	if (!addToQueue)
		return;
...
void QueueCommand::onLocomotionFail(CreatureObject* creature, uint32 actioncntr, uint32 actionCRC) {
	if (!checkInvalidLocomotions(creature))
		creature->clearQueueAction(actioncntr, 0, 1, creature->getLocomotion(), actionCRC);
}

/*
 * Unsuccessful command completion alerts the player of the invalid state
 */
void QueueCommand::onFail(uint32 actioncntr, CreatureObject* creature, uint32 errorNumber, uint32 actionCRC) {
	...
	case INVALIDSYNTAX:
		...
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 0, 0, actionCRC);

		break;
	case INVALIDSTATE:
		onStateFail(creature, actioncntr, actionCRC);
		creature->showFlyText("combat_effects", "wrong_state_fly", 0xFF, 0xFF, 0xFF);
		break;
	case INVALIDLOCOMOTION:
		onLocomotionFail(creature, actioncntr, actionCRC);
		break;
	case INVALIDTARGET:
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 3, 0, actionCRC);

		creature->showFlyText("combat_effects", "target_invalid_fly", 0xFF, 0xFF, 0xFF);
		break;
	case INVALIDWEAPON: // this only gets returned from combat commands
		...
		creature->sendSystemMessage("DEBUG: InvalidWeapon. Alert Dreadlow if you see this message.");
		break;
	case TOOFAR:
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 4, 0, actionCRC);

		creature->showFlyText("combat_effects", "range_too_far", 0x02, 0x84, 0x82);
		break;
	case NOJEDIARMOR:
	...
	case NOPRONE:
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 1, 7, actionCRC);

		creature->showFlyText("combat_effects", "wrong_posture_fly", 0xFF, 0xFF, 0xFF);
		break;
	case NOKNEELING:
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 1, 4, actionCRC);
	...
	case INSUFFICIENTHAM:
		...
		creature->showFlyText("combat_effects", "action_too_tired", 0x02, 0x84, 0x82);
		break;
	case TOOCLOSE:
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 0, 0, actionCRC);

		creature->showFlyText("combat_effects", "range_too_close", 0x02, 0x84, 0x82);
		break;
	case CANTSEE:
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 0, 0, actionCRC);

		creature->showFlyText("combat_effects", "cant_see", 0x02, 0x84, 0x82);
		break;
	case INSUFFICIENTFORCE:
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 0, 0, actionCRC);

		creature->showFlyText("combat_effects", "action_low_force", 0xFF, 0xD7, 0x00);
		break;
	default:
		if (addToQueue)
			creature->clearQueueAction(actioncntr, 0, 0, 0, actionCRC);

		creature->sendSystemMessage("DEBUG: Default path, errorNumber is: " + String::valueOf(errorNumber) + ". Alert Dreadlow if you see this message.");
		break;
	}
}

void QueueCommand::onComplete(uint32 actioncntr, CreatureObject* player, float commandDuration, uint32 actionCRC) {
	if (!player->isPlayerCreature())
		return;

	if (addToQueue)
		player->clearQueueAction(actioncntr, commandDuration, 0, 0, actionCRC);
}
