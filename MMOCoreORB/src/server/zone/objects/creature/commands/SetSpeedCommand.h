...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) {
		...
		try {
			...
			creature->sendSystemMessage("SYNTAX: /setSpeed <speed>");
			return INVALIDPARAMETERS;
		}

		if (speed > 1.f)
			creature->sendSystemMessage("Speed set to " + String::valueOf(speed) + ".");
		else
			creature->sendSystemMessage("Speed has been reset to default.");

		creature->updateSpeedAndAccelerationMods();

		creature->setSpeedMultiplierBase(speed, true);
