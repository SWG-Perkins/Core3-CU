...
: BaseLineMessage(tano->getObjectID(), objType, 6, opcnt) {
		...
		if (tano->isCreatureObject()){
			...
			StringId* id = tano->getObjectName();

			String detailedName = id->getFile();

			if (detailedName.contains("_name")) {
				detailedName = detailedName.replaceAll("_name", "_detail");
			}

			String detailedDescription = id->getStringID();

			if (detailedDescription.contains("_n")) {
				detailedDescription = detailedDescription.replaceAll("_n", "_d");
			}


			insertAscii(detailedName);
			insertInt(0);
			insertAscii(detailedDescription);
