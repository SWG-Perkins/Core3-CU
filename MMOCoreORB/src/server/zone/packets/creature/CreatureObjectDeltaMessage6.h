...
public:
	...
	void updateWeapon() {
		addLongUpdate(0x06, creo->getWeaponID());
	}

	void updateTarget() {
		addLongUpdate(0x0A, creo->getTargetID());
	}
	...
	void updateGroupID() {
		addLongUpdate(0x07, creo->getGroupID());
	}

	void updateGuildID() {
		addIntUpdate(0x09, creo->getGuildID());
		insertInt(0x0);
		insertInt(0x0);
		insertInt(0x0);
	}
	...
	void updateMoodID() {
		//addByteUpdate(0x05, creo->getMoodID());
		insertByte(0x05);
	...
	void updateVisible() {
		addByteUpdate(0x12, creo->getVisible());
	}
