...
public:
	...
	void insertAttribute(const String& attribute, float value, float value2) {
		StringBuffer t;
		t << value;

		UnicodeString Value = UnicodeString(t.toString());
		UnicodeString Value2 = UnicodeString(t.toString());

		insertAscii(attribute);
		insertUnicode(Value);
		insertUnicode(Value2);
